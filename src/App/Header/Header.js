      ...
      return (
        <Fragment key={i}>
          <li>
            ...
            <span className={style.span}></span>{text}</a>
          </li>
        </Fragment>
      );
    } else {
  ...
  return (
    <header className="bg-white shadow-[0_0px_10px_0px_#ddd] relative">
      <div className="container flex justify-between items-center flex-wrap relative">
        <a className={style.logo} href="#d1">{props.title}</a>
        <ul className="main-nav flex m-auto md:m-0">
          {mainLinks}
          <li className={otherLinksStyle} onClick={() => handleMegaMenu()}>
            <a className={style.link} href="#other-links">Other Links</a>
            <MegaMenu titles={otherLinks} icons={props.icons} img={props.img} sendMegaMenu={getMegaMenu} />
          </li>
        </ul>
      </div>
    </header>
